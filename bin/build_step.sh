...
declare -A BUILD_STEP_USAGE=(
	...
	[USAGE_EX]="__${BUILD_STEP_TOOL} main.c hello_world"
)
...
function __build_step() {
	...
	if [[ -n "${CF}" && -n "${OF}" ]]; then
		...
		__info_debug_message "$MSG" "$FUNC" "$BUILD_STEP_TOOL"
		__progress_bar PB_STRUCTURE
		declare -A config_build_step=()
		__load_conf "$BUILD_STEP_CFG" config_build_step
		STATUS_CONF=$?
		declare -A config_build_step_util=()
		__load_util_conf "$BUILD_STEP_UTIL_CFG" config_build_step_util
		STATUS_CONF_UTIL=$?
		...
		if [ $STATUS -eq $NOT_SUCCESS ]; then
			MSG="Force exit!"
			__info_debug_message_end "$MSG" "$FUNC" "$BUILD_STEP_TOOL"
			exit 129
		fi
		...
		eval "chmod 755 ${OF}"
		__info_debug_message_end "Done" "$FUNC" "$BUILD_STEP_TOOL"
		exit 0
	fi
	__usage BUILD_STEP_USAGE
	exit 128
}
